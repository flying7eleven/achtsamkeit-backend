...
async fn main() {
    ...
    let logging_level = match env::var("ACHTSAMKEIT_LOGGING_LEVEL") {
    ...
    info!(
        "Starting Achtsamkeit backend ({}, build with rustc {})...",
        env!("VERGEN_GIT_SEMVER"),
        env!("VERGEN_RUSTC_SEMVER")
    );

    // get the configuration for the database server and terminate if something is missing
    let database_connection_url = env::var("ACHTSAMKEIT_DB_CONNECTION").unwrap_or_else(|_| "".to_string());
    if database_connection_url.is_empty() {
        error!("Could not get the configuration for the database server. Ensure ACHTSAMKEIT_DB_CONNECTION is set properly");
        return;
    }

    // get the psk for the token signature
    let token_signature_psk = env::var("ACHTSAMKEIT_TOKEN_SIGNATURE_PSK").unwrap_or_else(|_| "".to_string());
    if token_signature_psk.is_empty() {
        error!("Could not get the token signature PSK. Ensure ACHTSAMKEIT_TOKEN_SIGNATURE_PSK is set properly");
        return;
    }
    ...
    let achtsamkeit_database_config: Map<_, Value> = map! {
    ...
    unset_environment_variable("ACHTSAMKEIT_DB_CONNECTION");
    unset_environment_variable("ACHTSAMKEIT_TOKEN_SIGNATURE_PSK");
